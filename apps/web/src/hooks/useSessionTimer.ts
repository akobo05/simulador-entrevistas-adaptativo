...
/**
 * Temporizador de sesion ascendente.
 *
 * - useRef almacena el intervalo para evitar re-renders innecesarios.
 * - Cleanup automatico al desmontar el componente.
 * - Agnostico al tema visual; usalo desde ChatRoom u ObserverRoom
...
export function useSessionTimer(autoStart = false): SessionTimerState {
  ...
  const startTick = useCallback(() => {
    if (intervalRef.current !== null) return; // ya esta corriendo
  ...
  // Auto-start opcional y cleanup al desmontar
  useEffect(() => {
